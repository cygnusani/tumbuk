...
export class AppProvider {
  ...
  constructor(storage: Storage, private sqlite: SQLite, private translate: TranslateService, private toasts: ToastController) {
    ...
    if (!this.isOpen) {
      ...
      this.sqlite.create({ name: "data.db", location: "default" }).then((db: SQLiteObject) => {
        ...
        db.executeSql("CREATE TABLE IF NOT EXISTS symptom (id INTEGER PRIMARY KEY, name TEXT, order_nr INTEGER)", []).catch(err => {
          console.log("Error: ", err);
        });
